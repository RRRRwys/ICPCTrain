...
- cf1303F，2，2800，T，F，用并查集模拟一下操作，每次更新看当前新增了多少个连通块，正反都做一遍，结果作差就是当前次的答案 -- lsz
- AGC015D，2，900，F，T，“首先去除掉，全部相同的最高位，现在还有r位，将数字分为小于2^r的集合X和大于等于2^r的集合Y；如果数字全在X中，那么答案就是A到2^r-1；如果只选择Y中的数字，答案就是2^r到所有数或起来，可以考虑2^r,2^r+2^0,2^r+2^1,...2^r+2^k都小于B，因此可以构造2^r到2^r+2^(k+1)-1；如果X和Y都选，那么Y中选择2^r+A到2^r+2^r-1” -- wys
- AGC016B，1，700，T，F，“要求最大值最小值差不超过1，小的位置就是有一个单独的颜色，剩余的颜色的2倍要小于剩余的位置数，特判每个位置都是不同的情况即可” -- wys
- AGC016C，1，700，T，F，“首先显然每个小块最好都是-1，然后整个问题可以转化为一个大小为h*w的倍数的权值为很多-1的块放进去，剩余的块数是固定的，因此希望他们尽量的大，注意细节” -- wys

---

**2021-02-22**

- cf618f，3，3000，T，F，先来一波前缀和 然后如果a的和比b大 就swap一下 然后对a的每个位置 找到b里最后一个小于等于它的位置 然后把a-b记录一下 这样就有n个值了 并且他们也位于0-(n-1)之间 再加上a和b都是空集 就有n+1个值了 然后来一波抽屉原理 找到a1-b1==a2-b2 然后就知道a2+1到a1的和与b2+1到b1的和相同 就是答案了 -- zyp
- cf739e，3，3000，T，F，wqs二分 先二分a的惩罚值 然后check中二分b的惩罚值 然后将三种概率减去惩罚值dp即可 -- zyp 
- cf13e，2，2700，T，F，分块 对每一个位置记录它在下一个块的第一个位置 它在这个块的最后一个位置 以及它要到下一个块第一个位置需要的操作数 然后正常修改和查询就可以了 -- zyp
- cf605e，2，2700，T，F，类似于dijkstra 每次取出距离最短的点 然后对其他未访问的点做概率dp即可 -- zyp
- cf995f，2，2700，T，F，经过观察可以发现答案是一个关于d的最高项次数不超过n的多项式 如果d小于n就直接dp 不然就dp前n项 然后插值即可 -- zyp
- cf1421e，2，2700，T，F，观察发现最终序列中标记为-1的数目加上序列长度模三为1 因此贪心的标记-1 另外要检查一下最终序列标记是否是-1和1间隔 如果是的话 就需要在最后三个标记为1的东西附近微调 然后取最大值即可 -- zyp
- AGC019B，1，500，F，T，“当一个子串两头是相同的字符时，交换它内部这个串，与交换它本身一致，然后要发现如果两头不同，那就会构成不同的串；因此答案就是，统计 i < j 且 Ai != Aj的数目” -- wys
- cf504d，2，2700，T，F，贪心的每次选已有的数都尽可能的让得到是数更小 如果最终得到的数是0 就输出答案 不然就将得到的数以及组成这个数的方式存进数组即可 -- zyp
- AGC016D，3，1000，F，T，“题意是n个数字a1...an每次用所有数的xor和，替换其中一个元素，问最最少操作多少次，可以使得a序列变为b序列。考虑操作后整个序列的xor，就是选定的那个值，而继续操作就相当于用之前选定的元素再去替换，因此我们序列中所有元素的xor，作为a0，每次操作就相当于选定一个ai与a0交换，将a转化为b，最少的操作次数。我们尝试将a1...an中的每个元素与b1...bn连边，可以知道每个元素的入度出度差值至多是1，因为最多有一个元素在被选定为a0，而欧拉路的性质是：所有节点的出度等于入度；或者除两个节点以外的其余节点的入度和出度都相等，其余点一个多1，一个少1，可以发现对于每个连通块，它一定存在欧拉路；我们将a0这个元素在图中的节点，想成我们当前的位置，而我们沿着一条边走过去时，就是交换2个元素，而当我们沿着欧拉路，走完整个联通块的时候，我们就将所有的连通块内部的元素放到了正确的位置；接下来我们当前的位置不能到达任何一个点，因此需要通过一次操作走到，某一个连通块中；接下来，重复上面的操作，走欧拉回路；因此，最后的答案就是，总的边数+连通块数-1；一个特例是，如果我们一开始的位置的值，在所有连通块中都没出现过，要多用1次跳到进入某一个连通块。这个过程我的一个理解是，你可以想成每次你把a0这个元素换到一个它应该在的地方，重复这个操作直到无法操作时就相当于走完了欧拉回路，接下来，你就随便找一个位置还有问题的元素跟他再交换一下即可，然后就能继续重复上面的操作。神仙题” -- wys
- AGC018B，1，700，T，F，“考虑一开始有所有的运动，那么当前最大的人数对应的运动，如果不去掉，这些人就不能拆开，于是删掉这个运动；重复这个过程，直到删完所有运动，同时维护答案” -- wys

---

**2021-03-01**

- AGC018C，2，800，T，F，“只考虑金和银，将所有的人按照Ai-Bi排序，那么最乐观的情况下，当然是金牌都在银牌的右边；如果存在一个金牌在银牌左边，那么显然交换一下肯定更优，也就是说一定存在一个K，使得排序后的前K个不是选金就是选铜，剩余的不是选银就是选铜，这就转化为了只有2种牌的情况了，枚举K同时维护答案” -- wys
- AGC017C，3，1000，F，T，“维护每个值出现的次数，然后就可以发现，当一个位置i是w，那么他超前w-1个数都是0，也就是它决定了[i-w+1, i]的数字，而一个合法的解，这些区间一定恰好覆盖整个区间；接下来，要想到对每个值w它对应的区间就是[i-w+1, i]，但是有可能出现区间相交，我们知道每个球代表一个单位的覆盖，因此，我们就可以将重复覆盖的区间，移动到未覆盖的区间，因此答案就是未覆盖的区间；维护未覆盖的区间数和每个位置被覆盖的次数即可” -- wys
- AGC019C，2，900，T，F，“仔细看数据条件。任意2点之间的x、y坐标均不相同，那么一般情况下答案就是对于围起来的点做LIS，然后每过一个圆贡献5 * pi-20，但是只有一种特殊情况，就是当LIS穿过整个区域，那么最后一个点的贡献就是 10 * pi-20” -- wys
